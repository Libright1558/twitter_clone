...
const getPost = async (req, res) => {
    try {
        ...
        for (let i = 0; i < userPostsLen; i++) {
            ...
            PostsIdArray.push(userPosts[i].postId);
        }
        ...
        if (PostsIdArray?.length !== 0) {
            promise.push(setPostIdArray(userId, PostsIdArray));
        } 

        if (userPostsLen !== 0) {
            promise.push(postWriteBack(userId, postNestObj, fetchList));
        }
