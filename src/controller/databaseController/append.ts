...
import { PostData, PostUsername } from '.';
...
* }
*/
const insertPost = async (postData: PostData) => {
    try {
...
* }
*/
const appendLike = async (param: PostUsername) => {
    try {
...
* }
*/
const appendRetweet = async (param: PostUsername) => {
    try {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
