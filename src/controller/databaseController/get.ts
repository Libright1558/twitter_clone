...
* 0: absent, 1: present
*/
const postInfo = async (username: string, fetchList: FetchListArray) => {
    try {
...
const selfLikeInfo = async (postId: string, username: string) => {
    try {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query('ROLLBACK');
            console.log('selfLikeInfo rollback: ', error);
        } finally {
            client.release();
        }
    } catch (error) {
...
const selfRetweetInfo = async (postId: string, username: string) => {
    try {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query('ROLLBACK');
            console.log('selfRetweetInfo rollback: ', error);
        } finally {
            client.release();
        }
    } catch (error) {
