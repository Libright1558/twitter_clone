import { RedisPostIdArray, RenewLikeRetweetNum, RedisClientType } from ".";
import { PersonalDetail, FetchListArray, PostArrayNest } from "..";

const getUserInfo = async (client: RedisClientType, userId: string) => {
...
const getPostInfo = async (client: RedisClientType, userId: string, postIdArray: RedisPostIdArray) => {
...
const getPostIdArray = async (client: RedisClientType, userId: string) => {
    const result = await client.LRANGE(userId + '_postIdArray', 0, -1);
    return result;
};

const writeUserInfo = async (client: RedisClientType, userId: string, userInfo: PersonalDetail) => {
    await client
        .multi()
...
* 0: absent, 1: present
*/
const writePostInfo = async (client: RedisClientType, userId: string, postNestObj: PostArrayNest, listArray: FetchListArray) => {
    if (!listArray[0]) {
...
// member is an array
const writePostIdArray = async (client: RedisClientType, userId: string, member: RedisPostIdArray) => {
...
* }
*/
const renewLikeNums = async (client: RedisClientType, postId: string, obj: RenewLikeRetweetNum, expTime: number) => {
    await client
        .multi()
...
* }
*/
const renewRetweetNums = async (client: RedisClientType, postId: string, obj: RenewLikeRetweetNum, expTime: number) => {
    await client
        .multi()
...
const deletePostInfo = async (client: RedisClientType, userId: string, postId: string) => {
    await client
        .multi()
...
const setUserInfoExp = async (client: RedisClientType, times: number) => {
    await client
        .multi()
...
const setPostInfoExp = async (client: RedisClientType, userId: string, times: number) => {
    await client
        .multi()
